...
public class OverlayManager {
    ...
    public var isUserInteractionEnabled: Bool {
        get {
           // return self.singleTapGestureRecognizer.view != nil
            return self.rightSwipeTapGestureRecognizer.view != nil
            
        }

        set {
            self.overlayView.addSubview(buttonNext)
            let constraints = [
            buttonNext.trailingAnchor.constraint(equalTo: overlayView.trailingAnchor, constant: -35),
            buttonNext.bottomAnchor.constraint(equalTo: overlayView.bottomAnchor, constant: -35)
            ]
            ...
            self.overlayView.addGestureRecognizer(self.rightSwipeTapGestureRecognizer)
            if newValue == true {
                self.overlayView.addGestureRecognizer(self.leftSwipeTapGestureRecognizer)
                self.overlayView.addGestureRecognizer(self.downSwipeTapGestureRecognizer)
               
            } else {
                self.overlayView.removeGestureRecognizer(self.leftSwipeTapGestureRecognizer)
                self.overlayView.removeGestureRecognizer(self.downSwipeTapGestureRecognizer)
    ...
    private lazy var rightSwipeTapGestureRecognizer:UISwipeGestureRecognizer = {
        let gesture = UISwipeGestureRecognizer()
        gesture.direction = .right
        gesture.addTarget(self, action: #selector(handleSingleTap(_:)))
        return gesture
    }()
    private lazy var leftSwipeTapGestureRecognizer:UISwipeGestureRecognizer = {
        let gesture = UISwipeGestureRecognizer()
        gesture.direction = .left
        gesture.addTarget(self, action: #selector(leftSwipeSingleTap(_:)))
        return gesture
    }()
    private lazy var downSwipeTapGestureRecognizer:UISwipeGestureRecognizer = {
        let gesture = UISwipeGestureRecognizer()
        gesture.direction = .down
        gesture.addTarget(self, action: #selector(downSwipeSingleTap(_:)))
        return gesture
    }()
    public lazy var buttonNext: UIButton = {
        let button = UIButton()
        button.setTitle("Далее", for: .normal)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 14)
        button.titleLabel?.textAlignment = .left
        button.translatesAutoresizingMaskIntoConstraints = false
        button.addTarget(self, action: #selector(handleSingleTap(_:)), for: .touchUpInside)
        return button
    }()
    ...
    @objc private func handleSingleTap(_ sender: AnyObject?) {
        if enableTap {
            UserDefaults.standard.set(true, forKey: "swipe")
            self.overlayDelegate?.didReceivedSingleTap()
        }
    }
    @objc private func leftSwipeSingleTap(_ sender: AnyObject?) {
        if enableTap {
            UserDefaults.standard.set(false, forKey: "swipe")
            self.overlayDelegate?.leftSwipe()
        }
    }
    @objc private func downSwipeSingleTap(_ sender: AnyObject?) {
        if enableTap {
            UserDefaults.standard.set(false, forKey: "startTraning")
            self.overlayDelegate?.downSwipe()
        }
    }
